...
- Steps:
  - Convert point/quantity fields to `PositiveIntegerField` or add `CheckConstraint` where negatives are allowed (e.g., penalties) but need bounds.
  - Add `clean()` validations for `Reward` and `Chore` point ranges; keep `save()` logic to backfill `quantity_remaining` on create.
  - Implement an invite-code regeneration loop to guarantee uniqueness on `Household` creation/regeneration.
  - Add DB indexes: `Chore(household, status, priority)`, `ChoreInstance(household via chore, status, due_date)`, `Leaderboard(household, period)`, `PointTransaction(user, created_at)`.
  - Create and run migrations; add minimal model tests for constraints and invite-code uniqueness.
...
- Steps:
  - Create `services/` package with modules: `points.py` (award/spend, transaction logging, streak updates), `chores.py` (state transitions, verification), `notifications.py` (enqueue/send).
  - Move logic from views/serializers into services with idempotent functions; return typed results (e.g., dataclass with updated models).
  - Add unit tests around services to cover chore completion, transfers, reward redemption, and error cases.
...
- Steps:
  ...
  - Add invite/join endpoints and simple UI to accept invite codes; prevent membership enumeration.
  - Provide a lightweight login/register page (or API) instead of routing users to `/admin/`.
...
- Steps:
  - Remove default `SECRET_KEY` and document required env vars; expand `.env.example`.
